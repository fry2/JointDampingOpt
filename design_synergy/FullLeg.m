...
%             axesMat(:,3) = axis_angle_rotation(obj,theta(2),axesMat(:,2))*obj.CR_bodies(:,:,1)*obj.CR_bodies(:,:,2)*obj.CR_bodies(:,:,3)*obj.CR_bodies(:,:,4)*obj.three_axis_rotation(obj.joint_obj{3}.euler_angs)*[-1;0;0];
        ...
        function store_torque_and_kinematic_data(obj,theta,theta_dot)
            ...
            if ismac
                ...
                load('G:\My Drive\Rat\SynergyControl\Data\AllTraining20180824.mat','TorqueAll');
            end
            torque = TorqueAll;
            for i=1:length(obj.body_obj)-1
...
%                 axesMat(:,i) = obj.CR_bodies(:,:,i+1)*obj.three_axis_rotation(obj.joint_obj{i}.euler_angs)*[-1;0;0];
        ...
        function store_animatlab_params( obj )
            ...
            load(['G:\My Drive\Rat\SynergyControl\Data\AnimatlabProperties.mat']);
        ...
        function store_johnson_params(obj)
            params = [];
            num_musc = length(obj.musc_obj);
            load(['G:\My Drive\Rat\SynergyControl\Data\Johnson2011MuscleParameters.mat'],'params');
...
%                             grid on
        ...
        function [passive_joint_torque,passive_joint_motion,passive_muscle_torque] = compute_passive_joint_torque(obj)
         num_muscles = length(obj.musc_obj);
         ...
         for k = 1:3   
            ...
            for i = 1:length(relevant_muscles{k})
                ...
                pt = smoothdata(musc.passive_tension,'movmedian');
                %pt = musc.passive_tension;
                n = length(pt);
...
%                             end
        ...
        function [force,force2actuate,tau2,moment_output,fvals,telapsed,indiv_torques,exitflag] = optimize_forces(obj,fun,toplot)
            ...
            % Filter the passive_torque waveform
            passive_torque = smoothdata(passive_torque,'loess');
            ...
            for i=1:nummuscles
                ...
                mV = smoothdata(mV(1:end-10),'loess');
                mV = [mV;repmat(mV(end),10,1)];
                mVees(i,:) = mV;
                Lr = muscle.RestingLength;
                Lw = muscle.l_width;
                Al_musc = fl(mL,Lr,Lw);
            ...
            options = optimoptions('fmincon','Display','none','Algorithm','sqp','OptimalityTolerance',1e-2);
            exitflag = -10.*ones(1,length(xx));
            fvals = -1.*ones(1,length(xx));
            ...
            for j = 1:length(xx)
                ...
                if j>1
                    x0 = force(:,j-1);
                    fun = @(x) sum((x./Fmax).^2)+.003*sum((x-x0).^2);
                else
                    fun = @(x) sum((x./Fmax).^2);
                end
                lb = max(lbBnd(:,1).*x0+lbBnd(:,2).*lbHelper(:,j),0);
                [force(:,j),~,exitflag(j)] = fmincon(fun,x0,[],[],Aeq,beq,lb,ub,[],options);
                fvals(j) = sum(Aeq*force(:,j)-beq');
                lbMat(:,j) = lb;
                bb = 1;
                %disp(num2str(count))
                count = count +1;
